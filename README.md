...
[![codecov](https://img.shields.io/codecov/c/github/retr0h/mlb-mcp?style=for-the-badge)](https://codecov.io/gh/retr0h/mlb-mcp)
...
[Model Context Protocol (MCP)][mcp] is an open standard that lets LLMs call
external tools and data sources in a structured, type-safe way. This server
wraps the [mlb-sdk] Go library and exposes MLB Stats API endpoints as MCP tools
that any MCP-compatible LLM (Claude, GPT-4, etc.) can call directly. No API key
required — MLB's Stats API is public.
...
verified. Override with `MLB_MCP_INSTALL_DIR=/some/path` or pin a version with
`MLB_MCP_VERSION=1.0.0`.
...
This repo ships a [`.mcp.json`](.mcp.json) that Claude Code auto-discovers when
...
Any client that supports stdio transport can launch `mlb-mcp mcp start` as a
subprocess. See [`.mcp.json`](.mcp.json) for the canonical config.
...
They accept friendly arguments and return clean, structured JSON from the typed
[mlb-sdk] Go library.
...
Every other endpoint from the [mlb-sdk] OpenAPI spec is auto-generated at build
time by `mcpgen` (run via `go generate ./internal/mcp/`). These tools pass
parameters directly to `statsapi.mlb.com` and return raw JSON. When a new
endpoint is added to mlb-sdk, regenerating picks it up automatically.
...
This server wraps the [mlb-sdk] Go library, which in turn wraps MLB's public
Stats API (`statsapi.mlb.com`). The composed tools call the typed SDK through a
`Driver` interface; the raw tools call the API directly with parameters derived
